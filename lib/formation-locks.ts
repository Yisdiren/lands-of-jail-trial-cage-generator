...
export function buildLockedFormations(pool: Hero[], count: number, locks: Locks, plan: TroopPlan, levels: WarSkillLevels, robots: string[], verified: boolean, leader: Formation | null, mode: "leader" | "joiner"): Formation[] {
  const reserved = mode === "joiner" && leader ? new Set(slots.map(s => leader[s].name)) : new Set<string>();
  const allowed = pool.filter(h => h.cageAllowed && !reserved.has(h.name) && (mode === "leader" || h.rarity === "SSR" || ["Lunarl","Lofili","Samir"].includes(h.name)));
  const active = Object.entries(locks).filter(([key, name]) => name && Number(key.split(":")[0]) < count);
  ...
  const used = new Set<string>(); const output: Formation[] = [];
  for (let i=0;i<count;i++) {
    ...
    function search(pos: number): boolean {
      ...
      candidates.sort((a,b) => pos===0 && mode==="joiner" ? ((b.leftTier==="top"?300:b.leftTier==="strong"?200:100)+(b.leftValue??0))*(levels[b.name]??5)-((a.leftTier==="top"?300:a.leftTier==="strong"?200:100)+(a.leftValue??0))*(levels[a.name]??5) : Number(!!a.leftSkill)-Number(!!b.leftSkill) || a.name.localeCompare(b.name));
      for(const h of candidates){chosen.push(h);if(search(pos+1))return true;chosen.pop();}
      return false;
    }
    ...
    chosen.forEach(h=>used.add(h.name));
    const level=levels[chosen[0].name]??5;
