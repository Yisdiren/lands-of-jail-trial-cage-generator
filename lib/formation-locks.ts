...
export function buildLockedFormations(pool:Hero[],count:number,locks:Locks,levels:WarSkillLevels,robots:string[],verified:boolean,leader:Formation|null,mode:"leader"|"joiner",stars:HeroStarLevels={}):Formation[]{
  ...
  for(let i=0;i<count;i++){
    ...
    if(!search(0)){if(active.some(([k])=>Number(k.split(":")[0])>=i)){const marchLocks=active.filter(([k])=>Number(k.split(":")[0])===i).map(([k,n])=>`${k.split(":")[1].toUpperCase()}=${n}`).join(", ");throw new Error(`Lock conflict on ${mode==="leader"?"MAIN":"J"+(i+1)}${marchLocks?` (${marchLocks})`:""}: no legal Shield/Bomber/Shooter combination remains.`);}break}
