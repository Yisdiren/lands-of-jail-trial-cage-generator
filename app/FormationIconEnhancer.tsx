...
function generalizeStaticCopy(){document.querySelectorAll<HTMLElement>("h1,h2,h3,label,p,button").forEach(el=>{if(!el.childElementCount&&el.textContent?.includes("CCW"))el.textContent=el.textContent.replace(/CCW\s*/g,"")});const badge=document.querySelector<HTMLElement>(".badge");if(badge)badge.textContent="BETA v0.37"}
...
export default function FormationIconEnhancer(){useEffect(()=>{enhanceGenerator();guideRosterAfterWizard();guideTroopsAfterRoster();const observer=new MutationObserver(enhanceGenerator);observer.observe(document.body,{childList:true,subtree:true});return()=>observer.disconnect()},[]);return null}
