...
const defaults={ratios:{shield:0,bomber:0,shooter:100}as TroopValues,tiers:{shield:"T10",bomber:"T10",shooter:"T10"}as TroopTiers};
...
function planFor(p:SavedProfile){const leader=p.role==="leader",capacity=leader?(p.leaderCapacity??100000):100000,ratios=leader?(p.leaderRatios??defaults.ratios):(p.joinerRatios??defaults.ratios);return calculateTroopPlan({capacity,ratios,tiers:p.troopTiers??defaults.tiers})}
...
function guideTroopsAfterRoster(){
  ...
  const tiers={...(p.troopTiers??defaults.tiers)};
  ...
  card.innerHTML=`<div style="font-size:12px;opacity:.65">ACCOUNT SETUP — FINAL STEP</div><h2>Robots and troops</h2><p style="opacity:.76">Choose the robots you own and your troop tiers. Troop counts are calculated from march capacity and your chosen ratio.</p><h3>Robots</h3><div data-robots style="display:flex;flex-wrap:wrap;gap:8px"></div><h3 style="margin-top:18px">Troops</h3><div data-troops style="display:grid;gap:8px"></div><div style="display:flex;justify-content:space-between;margin-top:20px"><button data-skip type="button">SKIP</button><button data-save type="button" style="font-weight:800">SAVE & OPEN GENERATOR</button></div>`;
  ...
  const troopHost=card.querySelector<HTMLElement>("[data-troops]")!;(["shield","bomber","shooter"] as const).forEach(k=>{const row=document.createElement("div");row.style.cssText="display:grid;grid-template-columns:120px 1fr;gap:8px;align-items:center";const label=k==="shield"?"Shieldbearer":k[0].toUpperCase()+k.slice(1);row.innerHTML=`<b>${label}</b><select data-tier="${k}" style="padding:8px;background:#18202d;color:inherit">${Array.from({length:11},(_,i)=>`<option value="T${i+1}" ${tiers[k]===`T${i+1}`?"selected":""}>T${i+1}</option>`).join("")}</select>`;troopHost.append(row)});
  const save=()=>{card.querySelectorAll<HTMLSelectElement>("[data-tier]").forEach(el=>tiers[el.dataset.tier as keyof TroopTiers]=el.value as TroopTiers[keyof TroopTiers]);ps[index]={...p,ownedRobots:[...selectedRobots],troopTiers:tiers,updatedAt:Date.now()};localStorage.setItem("loj-member-profiles-v1",JSON.stringify(ps));localStorage.setItem(`loj-account-onboarding-v036:${p.id??"player"}`,"complete");window.location.reload()};
